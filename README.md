...
* **Provenance verified, not asserted:** all **31/31** submitted snippets were re-located inside the shipped OCR, and **29/31** reproduce the declared `bbox` to within 0.002. The remaining two are **ambiguous by construction** — the phrase occurs twice on the same page and no available criterion picks between the two occurrences (deltas 0.3888 and 0.4034). Two others *were* wrong and have been corrected: one clipped the first character of its line, the other omitted the last line of the very quotation it was meant to support. Both were caught by drawing the declared and the recomputed box on the rendered page — the same image-first rule the rest of the pipeline follows. The brief's own reading criteria include *"whether the boxes point where you say they do"*, which is why this is broken down rather than bundled. Three of the events cite **Hadean's** filings rather than Archean's — the check searches the whole corpus, not just the subject's folder, because that is where the evidence is. Reproduce with `python main.py --siren 480489707 --benchmark results.json`.
