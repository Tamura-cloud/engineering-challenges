...
* **Provenance verified, not asserted:** all **31/31** submitted snippets were re-located inside the shipped OCR, and **27/31** reproduce the declared `bbox` to within 0.002. Of the remaining four, **two are ambiguous by construction** — the phrase occurs twice on the same page and no available criterion picks between the two occurrences (deltas 0.3888 and 0.4034) — and **two are a plain imprecision of about one line**, with no ambiguity in the source at all (deltas 0.0113 and 0.0171). They are not the same thing, and bundling them under "documented ambiguities" is what an earlier version of this README did. The brief's own reading criteria include *"whether the boxes point where you say they do"*, which is why the breakdown is spelled out. Three of the events cite **Hadean's** filings rather than Archean's — the check searches the whole corpus, not just the subject's folder, because that is where the evidence is. Reproduce with `python main.py --siren 480489707 --benchmark results.json`.
